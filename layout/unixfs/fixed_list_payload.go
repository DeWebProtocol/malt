...
type FixedListPayloadWriter interface {
	...
	ApplyFixedListPayloadMutation(ctx context.Context, mut writer.SemanticMutation) (cid.Cid, error)
}
...
func MaterializeFixedListPayload(ctx context.Context, blocks StagedBlockStore, writer FixedListPayloadWriter, r io.Reader, totalSize uint64, chunkSize int) (cid.Cid, error) {
	...
	err := StreamPayloadChunks(r, chunkSize, func(chunk []byte) error {
	...
	mut, err := FixedListPayloadMutation(baseRoot, chunks, totalSize, uint64(chunkSize))
	if err != nil {
		return cid.Undef, err
	}
