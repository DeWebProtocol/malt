...
func TestCIDForBlockTypedCodec(t *testing.T) {
	got, err := cas.CIDForBlock(cas.Block{Data: []byte(`{"entries":["a.txt"]}`), Codec: testTypedCodec})
	if err != nil {
		t.Fatalf("CIDForBlock: %v", err)
	}
	if got.Prefix().Codec != testTypedCodec {
		t.Fatalf("codec = %x, want %x", got.Prefix().Codec, testTypedCodec)
	}
}
...
func TestPutBlocksFallbackPreservesOrder(t *testing.T) {
	...
	blocks := []cas.Block{
		{Data: []byte("first")},
		{Data: []byte("second"), Codec: testTypedCodec},
		{Data: []byte("third")},
	}
...
func TestPutBlocksNonRawWithoutTypedWriterReturnsError(t *testing.T) {
	_, err := cas.PutBlocks(context.Background(), rawOnlyWriter{}, []cas.Block{
		{Data: []byte("typed"), Codec: testTypedCodec},
	})
	if err == nil {
