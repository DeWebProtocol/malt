...
func TestHTTPServerSupportsTypedBlockPut(t *testing.T) {
	...
	blockCID, err := client.PutWithCodec(ctx, payload, testTypedCodec)
	if err != nil {
		t.Fatalf("put typed block: %v", err)
	}
	if blockCID.Prefix().Codec != testTypedCodec {
		t.Fatalf("codec = %x, want %x", blockCID.Prefix().Codec, testTypedCodec)
	}
	data, err := client.Get(ctx, blockCID)
	if err != nil {
...
func TestHTTPServerSupportsPutBatch(t *testing.T) {
	...
	req := cashttpapi.PutBatchRequest{Blocks: []cashttpapi.PutBatchBlock{
		...
		{Codec: testTypedCodec, Data: base64.StdEncoding.EncodeToString([]byte(`{"entries":["a.txt"]}`))},
	}}
	...
	if typed.Prefix().Codec != testTypedCodec {
		t.Fatalf("codec = %x, want %x", typed.Prefix().Codec, testTypedCodec)
	}
}
