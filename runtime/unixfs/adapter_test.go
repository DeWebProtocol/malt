...
import (
	...
	unixfs "github.com/dewebprotocol/malt/runtime/unixfs"
...
func newLayout(t *testing.T, chunkSize int) *unixfs.Adapter {
	t.Helper()
	return newLayoutWithBlocks(t, chunkSize, casmock.NewCAS())
}

func newLayoutWithBlocks(t *testing.T, chunkSize int, blocks cas.Client) *unixfs.Adapter {
	t.Helper()
	return newLayoutWithMapDecorator(t, chunkSize, blocks, nil)
}

func newLayoutWithMapDecorator(t *testing.T, chunkSize int, blocks cas.Client, decorate func(mapping.Semantics) mapping.Semantics) *unixfs.Adapter {
	t.Helper()
