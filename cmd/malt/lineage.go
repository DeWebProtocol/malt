...
func runLineageGet(cmd *cobra.Command, args []string) error {
	...
	rec, err := client.GetLineage(cmd.Context(), root.String())
	if err != nil {
...
func runLineageAncestors(cmd *cobra.Command, args []string) error {
	...
	client := mustDaemonClient()
	ancestors, err := client.LineageAncestors(cmd.Context(), root.String(), lineageMaxDepth)
	if err != nil {
		return daemonCommandError(err)
	}
...
func runLineageDescendants(cmd *cobra.Command, args []string) error {
	...
	client := mustDaemonClient()
	children, err := client.LineageDescendants(cmd.Context(), root.String())
	if err != nil {
		return daemonCommandError(err)
	}
...
func runLineageList(cmd *cobra.Command, args []string) error {
	client := mustDaemonClient()
	records, err := client.ListLineage(cmd.Context())
	if err != nil {
		return daemonCommandError(err)
	}
...
func runLineageCount(cmd *cobra.Command, args []string) error {
	client := mustDaemonClient()
	count, err := client.CountLineage(cmd.Context())
	if err != nil {
		return daemonCommandError(err)
	}
...
func truncateLineageRoot(root string) string {
	if len(root) <= 16 {
		return root
	}
	return root[:16] + "..."
