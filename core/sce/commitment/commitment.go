...
type Scheme interface {
	...
	Prove(commitment cid.Cid, arcs arcset.Snapshot, path string) (cid.Cid, []byte, error)
	...
	// Returns the new commitment CID.
	BatchUpdate(commitment cid.Cid, arcs arcset.Snapshot, updates map[string]struct {
		Old cid.Cid
		New cid.Cid
	}) (cid.Cid, error)

	// BatchProve generates proofs for multiple paths.
	BatchProve(commitment cid.Cid, arcs arcset.Snapshot, paths []string) (map[string]arcset.BatchProofEntry, error)
	...
	AggregateProve(commitment cid.Cid, arcs arcset.Snapshot, paths []string) (*arcset.AggregatedProof, error)
