...
// Manager provides concurrent-safe access to lineage records with persistence.
// Keys:
//
...
// NewManager creates a new lineage manager backed by the given KVStore.
...
func (m *Manager) Record(ctx context.Context, root cid.Cid, parent cid.Cid, arcCount int) error {
	...
	if err := m.recordLocked(rec); err != nil {
		return err
	}
...
// recordLocked persists a LineageRecord and updates the parent's children index.
// Must be called without m.mu held (it acquires its own lock for KV writes).
func (m *Manager) recordLocked(rec *LineageRecord) error {
	if !rec.Root.Defined() {
	...
	// Store the record
	if err := m.kv.Set(lineageKey(rec.Root.String()), data); err != nil {
		return fmt.Errorf("store lineage record: %w", err)
	}

	// Update parent's children index
	if rec.Parent.Defined() {
		children, err := m.getChildren(rec.Parent.String())
		if err != nil {
			return err
		}
		...
		if err := m.setChildren(rec.Parent.String(), children); err != nil {
			return err
		}
	}
...
func (m *Manager) Get(ctx context.Context, root cid.Cid) (*LineageRecord, error) {
	...
	// Read from KV store
	data, ok := m.kv.Get(lineageKey(rootStr))
	if !ok {
...
func (m *Manager) Delete(ctx context.Context, root cid.Cid) error {
	...
	if rec.Parent.Defined() {
		...
		if err == nil {
			...
			_ = m.setChildren(rec.Parent.String(), newChildren)
		}
	}

	if err := m.kv.Delete(lineageKey(root.String())); err != nil {
		return err
	}
...
func (m *Manager) Descendants(ctx context.Context, root cid.Cid) ([]cid.Cid, error) {
	...
	return m.getChildren(root.String())
}
...
func (m *Manager) getChildren(parent string) ([]cid.Cid, error) {
	data, ok := m.kv.Get(childrenKey(parent))
	if !ok {
		return nil, nil
	}
...
func (m *Manager) setChildren(parent string, children []cid.Cid) error {
	data, err := json.Marshal(children)
	if err != nil {
		return fmt.Errorf("marshal children: %w", err)
	}
	return m.kv.Set(childrenKey(parent), data)
}
