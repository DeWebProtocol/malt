...
| 9b.2 | **Completed 2026-08-17:** add the concrete flat-v1/hybrid-v1 UnixFS client-root planner; reconstruct exact manifest/authenticated-binding projection, replay the complete ordered overlay, verify old/new manifest CIDs, and emit child-before-parent output-free intent for local KZG/IPA computation | additive experimental `unixfs/clientroot` planner API | no | incorrect ancestor projection, incomplete-view candidate, manifest CID substitution, or backend/object-identity drift | flat/hybrid write/mkdir/rename/unlink and new-directory combined replay, independently verified next views under KZG/IPA, corrupt old manifest, malicious returned manifest CID, immutable-View and frozen-status rejection, and race tests | leave the planner and generic orchestrator uncomposed; existing journal intent remains recoverable |
