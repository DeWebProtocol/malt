...
- `filesystem/staging` layers locally durable read-your-writes state over that
  ...
  pins its native open local handles to a CID, reconciles cross-store crash
  edges on startup, labels fsync as local-journal durability only, and freezes
  exact retry batches with atomic completion/conflict classification. It has
  no transport or trust capability; the outer runtime binding supplies those
  ports when an explicit write-back mount is selected.
...
- `filesystem/mount` persists desired mounts and pending-unmount tombstones,
  ...
  that adapter a filesystem already bound to a locally accepted View. The
  default is read-only; an explicit write-back policy may receive one
  session-owned writable binding. Unsupported lock targets fail closed before registry access. The
...
The platform-neutral filesystem boundary owns lookup, verified stat/read,
range reads, staging, rename/unlink semantics, dirty state, fsync policy,
offline journal, and conflicts. FUSE/WinFsp/platform packages only translate
OS operations into this service. A read may return bytes only after path proof,
...
- [x] **Completed 2026-08-23 as a decision gate, not a cutover:** retain
  `github.com/dewebprotocol/malt-client`, document the isolated pre-v1 cutover
  requirements, protect all tag refs with an active no-bypass GitHub ruleset,
  and make CI fail closed in a tag context until that dedicated change is
  reviewed and proven.
...
| 11 | **Completed 2026-08-23 as a decision gate, not a namespace cutover:** retain `github.com/dewebprotocol/malt-client`; document the two viable namespace strategies, disjoint version-line and isolated-proxy requirements; protect every tag ref with an active no-bypass repository ruleset; add an executable module invariant and defense-in-depth tag-context CI rejection before heavy builds | no current API change; the future dedicated cutover is a breaking Go import change | no | old-commit tag bypass, accidental runtime tag under the temporary path, collision with historical Core versions, or treating repository rename as module authorization | live ruleset inspection, architecture invariant, branch and real GitHub tag-environment gate tests, ordinary branch/PR CI, and future isolated `GOMODCACHE` proxy/direct consumer builds at cutover | keep the old module and do not tag; retain the ruleset, revert only gate documentation if its process is superseded, and never rewrite historical tags |
| 12 | **Completed 2026-08-23:** replace the pre-release tar/gzip backup object with runtime-owned `malt.encrypted-unixfs/v1`: encrypted dataset/directory/file manifests, stable opaque Map tokens, XChaCha20-Poly1305 file chunks, raw/List range reads, Plan-exclusive owner-local snapshot staging and KZG/IPA root computation, and one verified reader for restore, daemon, mount, local API, and browser adapters; migrate local `archive_name` configuration to `path_name` | pre-v1 source break in `PlanServiceOptions`, `Binding`, `Result`, and backup filesystem capabilities; local Plan/history JSON reads legacy fields and writes the new `path_name`/`profile` fields | new runtime application profile; no MALT Core wire, Root, CID, ProofList, mutation, or receipt change | plaintext-name disclosure, token drift on key rotation, decryption before proof/CID verification, remote root substitution, unaccepted-base reuse, rooted-path escape, stale-manifest reuse, silent old-profile misread, or unbounded crash-leftover snapshot storage | profile build/read/readdir/range/restore, exact KZG/IPA publication, malicious Map/List result, stable-token rotation, wrong key, corrupt CID/body, invalid proof, workspace gate before local snapshot, Plan-isolated crash-leftover and Close-failure recovery, unaccepted base, metadata drift, invalid UTF-8, escaping symlink, config migration, full test/vet/build, race, Windows build, and Gateway product E2E | revert code and use the previous binary only for old-profile roots; before upgrading a branch whose only copy uses the pre-release archive profile, restore it with the previous binary and republish from plaintext. New profile roots intentionally have no old-profile decoder in this tree |
