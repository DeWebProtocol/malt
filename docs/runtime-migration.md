...
| 9c.2 | Map Linux FUSE create/write/truncate/mkdir/rename/unlink/rmdir and local fsync onto the opt-in writable capability | experimental Linux adapter API | no | host syscall acknowledged before local durability | node/handle mutation, concurrent offset write, flush/fsync, errno, and kernel smoke tests | keep `read_only` default and reject writable mounts when capability is absent |
