...
- `filesystem/staging` layers locally durable read-your-writes state over that
  ...
  no transport or trust capability and is not yet composed into the daemon
  writable binding used by FUSE.
...
| 9c.3 | Compose per-dataset/branch staging, UnixFS planner, Gateway client-root write-back, candidate recording, and daemon lifecycle; make fsync remote semantics explicit | additive runtime config and composition | no | shared state lease collision, remote success mistaken for acceptance, or journal loss | daemon/foreground parity, candidate/no-change/conflict/offline/crash/remount and Gateway E2E | disable writable policy; retain per-view journal and candidates for recovery |
