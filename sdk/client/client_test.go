...
func TestClientMetricsSnapshotAndReset(t *testing.T) {
	...
	if reset.Snapshot != (httpapi.MetricsSnapshot{}) {
...
func TestClientRestartSafety(t *testing.T) {
	...
	firstNode, err := node.NewNode(node.WithConfig(cfg))
	if err != nil {
		t.Fatalf("create first node: %v", err)
	}
	ts := httptest.NewServer(server.New(firstNode, "127.0.0.1:0").Handler())
	cfg.RPC.Listen = ts.Listener.Addr().String()
	client := New(cfg)
	ctx := context.Background()
	...
	restartedNode, err := node.NewNode(node.WithConfig(cfg))
	if err != nil {
