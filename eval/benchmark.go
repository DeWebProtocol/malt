...
func (b *BenchmarkRunner) runAppendWorkload(ctx context.Context, arcCount int) (*Metrics, error) {
	...
	start := time.Now()
	start = time.Now()
	binding, proof, err := b.semantic.Prove(ctx, bucketID, root, arcset.CanonicalizePath(testPath))
...
func (b *BenchmarkRunner) runRandomWorkload(ctx context.Context, arcCount int) (*Metrics, error) {
	...
	// Initial commit
	start := time.Now()
	root, err := b.semantic.Commit(ctx, bucketID, mapping.NewViewFrom(arcsMap))
