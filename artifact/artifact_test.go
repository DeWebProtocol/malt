...
func TestResolvePayloadArtifactAllowsRootPathWithoutWeakeningIdentity(t *testing.T) {
	root := testCID(t, "root")
	...
	pl := prooflist.ProofList{
		...
		Query: "",
		Steps: []prooflist.Step{{
			Kind: prooflist.KindPayloadBinding, From: root, Path: "@payload", Target: payload,
		}},
	}
	req := artifact.ResolveRequest{Profile: artifact.Profile, Root: root.String(), Segments: []string{}}
	value, err := artifact.NewResolvePayloadArtifact(req, payload, pl)
	if err != nil {
		t.Fatal(err)
	}
	if err := artifact.Verify(context.Background(), artifact.VerifyRequest{
		Profile: artifact.Profile, Artifact: value,
	}, acceptingVerifier{}); err != nil {
		t.Fatalf("Verify(resolve_payload) failed: %v", err)
	}

	identity := value
	identity.Operation = artifact.OperationResolve
	identity.Target = root.String()
	if err := artifact.Verify(context.Background(), artifact.VerifyRequest{
		Profile: artifact.Profile, Artifact: identity,
	}, acceptingVerifier{}); err == nil {
		t.Fatal("resolve root identity accepted payload traversal evidence")
	}

	traversed := value
	traversed.ProofList.Steps = append([]prooflist.Step{{
		Kind: prooflist.KindMapStep, From: root, Path: "hidden", Target: root,
	}}, traversed.ProofList.Steps...)
	if err := artifact.Verify(context.Background(), artifact.VerifyRequest{
		Profile: artifact.Profile, Artifact: traversed,
	}, acceptingVerifier{}); err == nil {
		t.Fatal("empty resolve_payload query accepted hidden traversal evidence")
	}
}

func TestResolvePayloadArtifactBindsPayloadStepInsteadOfRangeTerminal(t *testing.T) {
	root := testCID(t, "root")
	fileRoot := testCID(t, "file-root")
	payload := testCID(t, "payload")
	segment := testCID(t, "segment")
	start, end := uint64(0), uint64(4)
	pl := prooflist.ProofList{
		Root: root, Query: "large.bin",
		Steps: []prooflist.Step{
			{Kind: prooflist.KindMapStep, From: root, Path: "large.bin", Target: fileRoot},
			{Kind: prooflist.KindPayloadBinding, From: fileRoot, Path: "@payload", Target: payload},
			{Kind: prooflist.KindListRange, From: payload, Start: &start, End: &end, Target: segment, EvidenceKind: "structure", EvidenceBackend: "measured_list"},
		},
	}
	req := artifact.ResolveRequest{Profile: artifact.Profile, Root: root.String(), Segments: []string{"large.bin"}}
	value, err := artifact.NewResolvePayloadArtifact(req, payload, pl)
	if err != nil {
		t.Fatal(err)
	}
	if err := artifact.Verify(context.Background(), artifact.VerifyRequest{
		...
		t.Fatalf("Verify(resolve_payload range) failed: %v", err)
	}
	value.Target = segment.String()
	if err := artifact.Verify(context.Background(), artifact.VerifyRequest{
		Profile: artifact.Profile, Artifact: value,
	}, acceptingVerifier{}); err == nil {
		t.Fatal("resolve_payload accepted the range terminal as its payload target")
	}
}
