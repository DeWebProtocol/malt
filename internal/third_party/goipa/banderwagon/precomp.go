...
func NewPrecompMSM(points []Element) (MSMPrecomp, error) {
	return newPrecompMSM(points, func(index int) int {
		if index < window16vs8IndexLimit {
			return 16
		}
		return 8
	})
...
// NewPrecompMSMWithWindowSize creates a fixed-base MSM table with one uniform
// window size. Smaller windows trade slower scalar multiplication for much
// lower retained memory. The window size must be a power of two that divides
// the 256-bit scalar width and must not exceed 16.
func NewPrecompMSMWithWindowSize(points []Element, windowSize int) (MSMPrecomp, error) {
	if err := validateWindowSize(windowSize); err != nil {
		return MSMPrecomp{}, err
	}
	return newPrecompMSM(points, func(int) int { return windowSize })
}

func newPrecompMSM(points []Element, windowSizeForIndex func(int) int) (MSMPrecomp, error) {
	...
	for i := 0; i < supportedMSMLength; i++ {
		windowSize := windowSizeForIndex(i)
		if err := validateWindowSize(windowSize); err != nil {
			return MSMPrecomp{}, fmt.Errorf("invalid window size for point %d: %w", i, err)
		}
...
func validateWindowSize(windowSize int) error {
	if windowSize <= 0 || windowSize > 16 || windowSize&(windowSize-1) != 0 || 256%windowSize != 0 {
		return fmt.Errorf("window size must be a power of 2 that divides 256 and is at most 16")
	}
	return nil
}
