...
func CreateMultiProof(transcript *common.Transcript, ipaConf *ipa.IPAConfig, Cs []*banderwagon.Element, fs [][]fr.Element, zs []uint8) (*MultiProof, error) {
	...
	D, err := ipaConf.CommitWithError(g_x)
	if err != nil {
		return nil, fmt.Errorf("could not commit quotient polynomial: %w", err)
	}
	...
	E, err := ipaConf.CommitWithError(h_x)
	if err != nil {
		return nil, fmt.Errorf("could not commit aggregated polynomial: %w", err)
	}
